...
- `codev` 负责把外部输入压成 repo 内 `tasks/`，并承接 task 分支内的执行技能。
...
- 半自动路径：`/office-hours -> /autoplan -> $gstack2task/$issue2task -> $autodev -> 人工确认 -> $automerge`
- 人工路径：按规划、任务、实现、验证、发布逐步运行各个 skill，由人类在关键节点做深度判断。
- repo 内的 `tasks/` 是 codev 的执行单元；`~/.gstack/projects/` 是 gstack 的上游工件目录。
