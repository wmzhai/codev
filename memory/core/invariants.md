...
- `codev-quickship` 负责人工验证后的 task 归档、任务相关 `docs/` / `memory/` / 必要时 `AGENTS.md` 同步，以及 commit / merge / push；若仓库已有版本号文件或 `CHANGELOG`，还要做最小同步；若 task 明确映射 GitHub issue，则主干 push 成功后还要用 `gh` 关闭对应 issue；但不自动新建版本工件、不打 tag、也不做正式发布。
