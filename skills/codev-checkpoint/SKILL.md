...
description: 轻量提交当前仓库变更并推送当前分支，不默认升级根目录 `VERSION`，也不默认同步 `CHANGELOG`。适用于用户明确要做一次 codev-checkpoint 提交，或仓库未接入 gstack `$ship` 时使用。
---
...
2. 仅当用户明确要求同步版本工件时：
   ...
   - 只接受单个、可稳定解析的 3 位纯数字版本号；如果当前仍是历史 4 位格式，则先停止并要求改由 `$codev-quickship` 或用户显式指定 3 位目标版本处理。
   - 如果用户显式指定目标版本，则按指定值写入，该版本同样必须是 3 位纯数字。
...
- 不做通用版本管理；默认不升级版本号。
- checkpoint 不默认升级版本号，也不默认同步 `CHANGELOG`。
- 如果用户显式指定目标版本，只接受 3 位纯数字版本号，并按指定值写入。
