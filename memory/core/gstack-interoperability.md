...
- `$codev-quickship` 负责人工验证后的收尾：归档 task、同步任务相关 `docs/` / `memory/` / 必要时 `AGENTS.md`，并在每次收尾时递增根目录 `VERSION`、同步 `CHANGELOG`，再提交、合并并推送主干；若 task 明确映射 GitHub issue，则在 push 成功后先补一条该轮工作的评论，再用 `gh` 关闭对应 issue；不调用 gstack 的正式发布技能。
