...
description: 轻量提交当前仓库变更并推送当前分支。适用于用户明确要做一次 codev-checkpoint 提交，或仓库未接入 gstack `$ship` 时使用。
---
...
如果仓库同时使用 gstack，默认发布入口应是 gstack `$ship`。`codev-checkpoint` 只保留为轻量 fallback：做 commit、push，但不接管 PR、review gate、QA 串联或 repo 级文档同步。
...
- 不做版本号、tag 或 release 管理；这不属于 codev-checkpoint 的职责。
