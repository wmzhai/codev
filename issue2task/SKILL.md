...
5. 任务拆分默认保守处理：
   ...
   - 单个 issue 产出的任务默认复用该 issue 编号作为基础任务号，例如 `#18` 默认生成 `T18`；如果拆成多个任务，则使用 `T18a`、`T18b`、`T18c`。
6. 创建或复用仓库根目录的 `tasks/`。
7. 保留已有任务为默认行为：
   - 如果任务来自单个 issue，默认直接使用该 issue 编号作为基础任务号；不要另起连续流水号。
   - 如果单个 issue 拆成多个任务，共用该 issue 编号并按 `a` / `b` / `c` 追加后缀。
   - 查找 `tasks/` 和 `tasks/done/` 中是否已存在相同任务编号；只有用户明确要求 regenerate 或 overwrite 时才覆盖已有任务文件。
   - 只有在多个 issue 被合并成一个任务、或 issue 编号对应的任务号已被无关历史任务占用时，才改用未占用的最小可用基础任务号，并在 `## Notes` 或 `tasks/README.md` 里写明映射原因。
8. 为每个任务写入 `tasks/T{nn}{suffix?}-{slug}.md`，模板如下：
...
9. 生成或刷新 `tasks/README.md`，用紧凑表格按统一任务顺序列出任务编号、标题、来源 issue 和依赖关系；任务顺序定义为基础号升序，再按后缀空值、`a`、`b`、`c` 升序。
...
- 同一 issue 如果拆成多个任务，必须共享该 issue 的编号并使用字母后缀，不要拆成连续不同整数号。
