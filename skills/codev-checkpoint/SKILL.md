...
description: 轻量提交当前仓库变更并推送当前分支；当用户明确要求同步版本时，可额外更新根目录 `VERSION` 并同步已有 `CHANGELOG`。若未显式指定版本，则默认把第 4 位加一。本仓库的标准版本工件是根目录 `VERSION` 和 `CHANGELOG`。适用于用户明确要做一次 codev-checkpoint 提交，或仓库未接入 gstack `$ship` 时使用。
---
...
如果仓库同时使用 gstack，默认发布入口应是 gstack `$ship`。`codev-checkpoint` 只保留为轻量 fallback：做 commit、push；只有用户明确要求同步版本时，才处理根目录 `VERSION` 与已有 `CHANGELOG`；如果用户要求同步版本但未显式指定版本，则默认按四段版本号 `x.y.z.w -> x.y.z.(w+1)`，也就是把第 4 位加一；但仍不接管 PR、review gate、QA 串联或 repo 级文档同步。
...
- 默认执行 commit + push。
- 如果用户明确要求同步版本，则只处理仓库里已经存在、且能稳定识别为 `x.y.z.w` 四段纯数字格式的根目录 `VERSION`；若未显式指定版本，则默认只把第 4 位加一；若显式指定目标版本，则按指定值写入；同时最小更新仓库里已有的 `CHANGELOG`，不新建版本文件，也不打 tag。
...
2. 如果用户明确要求同步版本：
   ...
   - 如果用户未显式指定版本，则默认把第 4 位加一，例如 `1.2.3.4 -> 1.2.3.5`。
...
- 不做通用版本管理；只有用户明确要求同步版本时，才允许处理仓库里已有的四段版本号。
- checkpoint 在需要同步版本但未显式指定版本时，默认把第 4 位加一。
