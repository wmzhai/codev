...
逐项核对任务文件中的验收标准，更新通过项的 checkbox；全部通过时归档到 `tasks/done/`。
...
它会优先做最小侵入验证，只在标准明确要求时运行测试或命令。遇到模糊标准时，应标记为需要人工确认，而不是猜测通过。
...
如果没有直接给 diff，也可以让它基于当前 patch 进行精简。`simplify` 同样是显式调用优先。
