...
5. 功能默认由人工验证；人工验证通过后，再用 `codev-quickship` 完成 task 归档、任务相关文档同步、版本工件同步，以及 commit / merge / push / tag；缺少 `VERSION` 或 changelog 时主动创建（首发 `0.0.1`）；用户触发即表示 taskdev 收尾校验和人工验证已完成，无 task 模式也依赖用户外部确认
