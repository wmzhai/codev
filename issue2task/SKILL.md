---
name: issue2task
description: 分析当前仓库的 GitHub Issues，结合现有代码理解需求，并生成 tasks/ 下的任务文件。适用于用户要先梳理需求、澄清范围、建立依赖顺序，再进入实现阶段的场景。
---
...
3. 仔细阅读 issue 的标题、正文、标签、里程碑和评论，然后大量阅读相关代码建立上下文：
   ...
   - 影响范围可能扩展到哪里。
4. 基于 issue 和代码现状，先向用户复述你的需求理解，再主动澄清高影响歧义：
   - 功能边界和 out-of-scope。
   - 关键用户场景。
   - 边界条件、异常流、权限和空状态。
   - 验收标准应该如何表述才可验证。
