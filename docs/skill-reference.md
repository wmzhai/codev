...
Purpose: 把 GitHub issue 或直接需求压成 repo 内 task 文件。
...
3. 决定要不要拆成多个 task。
4. 计算新任务编号。
5. 为每个 task 新建并切换到独立分支。
6. 写入 `tasks/T{nn}-{slug}.md`。

Produces / writes: 一个或多个 `tasks/T{nn}-{slug}.md`；对应的新分支。

Success output: repo 内可执行的 task 文件，足够让人工路径中的 `$plantask` 或半自动路径中的 `$autodev` 直接接手。

Stops / failure modes: 需要 issue 但 `gh` 不可用，或存在无法安全假设的关键需求歧义。

Next recommended steps: 人工路径走 `$plantask`；半自动路径直接进入 `$autodev`。

## `$gstack2task`

Purpose: 把 gstack 上游工件压成 repo 内 task 文件。
...
3. 收敛成一个或多个可独立执行的 task。
4. 编号顺延。
5. 为每个 task 新建独立分支并写入任务文件。

Produces / writes: 一个或多个 `tasks/T{nn}-{slug}.md`；对应的新分支。

Success output: repo 内 task 已建立，且保留了上游规划中的关键边界和测试重点。
...
Purpose: 把 task 文件进一步收敛成可直接实施的实现方案。

Typical trigger: 已有 `tasks/T{nn}-{slug}.md`，准备开始编码，但还需要一份落地计划。

Inputs / source of truth: 目标 task 文件、当前代码、依赖任务状态。

Preconditions: 需要目标 task 文件；依赖任务必须已经完成。

Git / branch state: 如果当前在 `main/master`，它会先切到 task 同名分支；不要求 clean tree；不改代码。

Reads prior outputs: task 文件、相关代码、前置任务完成状态。

Execution flow:
1. 选择目标 task。
2. 必要时切到 task 分支。
3. 读取任务背景、验收标准、依赖和相关代码。
4. 收敛实现路径和验证方案。
5. 在对话里输出一份详细实施计划。

Produces / writes: 对话中的实现方案；可能伴随分支切换，但不写 repo 文件。

Success output: 一份可以直接开始编码的计划。

Stops / failure modes: 没有待办 task，或依赖任务未完成。

Next recommended steps: 手动实现，或把这份方案交给 `$autodev` 继续推进。

## `$autodev`

Purpose: 在已有 task 分支上自动完成单个 task 的内部方案收敛、实现、验证、分支部署和 task 文档持续维护。
...
Reads prior outputs: task 文件、可选的 `$plantask` 方案（若人工路径先跑过）、相关 gstack review / QA 结果、部署配置。

Execution flow:
1. 选择 task，必要时切到 task 分支。
2. 检查依赖任务和分支部署前提。
3. 初始化 task 文档中的执行记录区。
4. 先做一轮内建的 `plantask` 式方案收敛，并写回 task 文档。
5. 分阶段实现、精简、提交、验证。
...
Next recommended steps: 人工确认部署结果；确认后运行 `$automerge`。如果不走半自动路径，也可以改回人工模式单独运行 `$plantask` / 实现 / 验证链。
...
- `issue2task` 与 `gstack2task` 是互斥入口
- 半自动路径里，`autodev` 内建 `plantask` 式方案收敛；手动路径里才单独运行 `plantask`
