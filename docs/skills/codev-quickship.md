...
在用户完成人工验证并确认结果后，完成 task 的统一收尾：默认以当前 git 分支为起点，归档到 `tasks/done/`、同步任务相关 `docs/` / `memory/` / 必要时 `AGENTS.md`，并同步根目录 `VERSION` 与已有 `CHANGELOG`；如果用户未显式指定版本，则默认按四段版本号 `x.y.z.w -> x.y.(z+1).0`，也就是把第 3 位加一并把第 4 位重置为 0；如果用户显式指定目标版本，则按指定值写入；然后再把当前工作状态提交、合并并推送到默认主干；如果 task 明确源自 GitHub issue，则在主干 push 成功后先在对应 issue 下追加一条本轮实际工作的摘要，再关闭对应 issue；提交时要使用 `type: 具体工作摘要 (vX.Y.Z.W)` 形式，工作摘要先写清具体工作，版本号放在最后括号里；不走 PR、不打 tag，也不接管正式发布流程。
...
4. 同步根目录 `VERSION` 与 `CHANGELOG`；如果用户未显式指定版本，则默认按 `x.y.z.w -> x.y.(z+1).0` 更新，也就是把第 3 位加一并把第 4 位重置为 0；如果用户显式指定目标版本，则按指定值写入；根目录 `VERSION` 缺失或格式不符时直接停止并说明仓库尚未初始化版本工件。
