---
name: ships
description: 轻量提交当前仓库变更并推送当前分支；如果用户提供版本号，则额外创建并推送 release tag。适用于用户明确要走轻量 commit/push/tag 流程，或仓库未接入 gstack `/ship` 时使用。
---
...
如果仓库同时使用 gstack，默认发布入口应是 gstack `/ship`。`ships` 只保留为轻量 fallback：做 commit、push、tag，但不接管 PR、review gate、QA 串联或 repo 级文档同步。
