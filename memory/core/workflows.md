...
## 默认主线
1. GitHub issue 或直接需求走 `codev-issue2task`；已有 `~/.gstack/projects/` 工件走 `codev-gstack2task`
2. 两个入口都要结合代码直接写出带实现计划的 `tasks/Txx-*.md`
...
7. 若需要正式发布链路，转入外部 gstack 发布流程；本文件不展开该链路
