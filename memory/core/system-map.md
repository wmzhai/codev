...
- `skills/codev-quickship/`：在用户完成人工验证后归档 task、同步任务相关 `docs/` / `memory/` / 必要时 `AGENTS.md`，并提交、合并、推送主干
...
- `codev-taskdev` 是手动实现层；`codev-autodev` 吸收 `codev-taskdev` 后再继续自动验证与分支部署；`codev-quickship` 负责人工验证后的手动收尾；`codev-automerge` 才进入正式发布与版本号。
