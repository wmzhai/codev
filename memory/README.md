...
`memory/` 是本仓库的公共记忆正文，回答“先看哪里、该改哪里、怎么验证”。Codex、Grok 和 Claude Code 都读这里；Codex 与 Grok 入口文件只负责专用前缀和跳转。Claude Code 没有项目入口文件。

## 热路径
1. 当前宿主入口：`AGENTS.md`（Codex）或 `.grok/rules/memory.md`（Grok）；Claude Code 直接读 `memory/`
...
- 当仓库结构或安装链路变化时，优先先更新这里，再回头修 Codex / Grok 入口、`README.md` 和测试脚本
