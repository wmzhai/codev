...
assert_host_installed() {
  ...
  skills_dir="$(skills_dir "$home_dir")"
...
exercise_default_codex_host() {
  ...
  assert_host_installed "$fresh_home"

  run_setup "$fresh_home"
  assert_host_installed "$fresh_home"

  codex_skills="$(skills_dir "$fresh_home")"
  ...
  mkdir -p "$(skills_dir "$conflict_home")/codev-issue2task"
  ...
  assert_exists "$(skills_dir "$conflict_home")/codev-issue2task"
  assert_missing "$(skills_dir "$conflict_home")/codev"
}
